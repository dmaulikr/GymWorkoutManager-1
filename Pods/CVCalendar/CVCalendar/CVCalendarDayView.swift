...
extension CVCalendarDayView {
    ...
    public func preliminarySetup() {
        if let delegate = calendarView.delegate,
            let shouldShow = delegate.preliminaryView?(shouldDisplayOnDayView: self)   where shouldShow {
    ...
    public func supplementarySetup() {
        if let delegate = calendarView.delegate,
            let shouldShow = delegate.supplementaryView?(shouldDisplayOnDayView: self)   where
            shouldShow {
    ...
    public func topMarkerSetup() {
        safeExecuteBlock({
            ...
            if let delegate = self.calendarView.delegate {
                ...
                if let shouldDisplay = delegate.topMarker?(shouldDisplayOnDayView: self)   where
                    shouldDisplay {
                        createMarker()
                }
            } else {
    ...
    public func moveDotMarkerBack(_ unwinded: Bool, coloring: Bool) {
        ...
        for dotMarker in dotMarkers {
            if let calendarView = calendarView, let dotMarker = dotMarker {
                var shouldMove = true
                if let delegate = calendarView.delegate,
                    let move = delegate.dotMarker?(shouldMoveOnHighlightingOnDayView: self)   where
                    !move {
                        shouldMove = move
                }
...
extension CVCalendarDayView {
    public func setSelectedWithType(_ type: SelectionType) {
        ...
        case .single:
            ...
            if let delegate = calendarView.delegate,
                let shouldShowCustomSelection = delegate.shouldShowCustomSingleSelection?()   where
